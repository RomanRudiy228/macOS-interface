...
export const Menubar = () => {
  ...
  useEffect(() => {
    ...
    if (rememberedUser?.username) {
      setDisplayUsername(rememberedUser.username);
    }
  }, []);
  ...
  const appleMenuItems = getAppleMenuItems(displayUsername, handleLogout);

  return (
    <header
      className="
        fixed top-0 left-0 right-0 z-50 h-9 w-full
        flex items-center justify-between px-4 select-none
        text-[13px] font-medium backdrop-blur-2xl shadow-sm transition-colors duration-300
        bg-white/40 text-black border-b border-black/5
    ...
    >
      <div className="flex items-center gap-4">
        <MenuBarItem
          ...
          className="hover:bg-black/5 dark:hover:bg-white/10 px-2 py-0.5 rounded transition-colors cursor-default"
        >
        ...
        <MenuBarItem
          ...
          className="font-bold text-black dark:text-white cursor-default hover:bg-black/5 dark:hover:bg-white/10 px-2 py-0.5 rounded transition-colors"
        >
          Finder
        </MenuBarItem>

        {/* Navigation Menu Items */}
        <nav className="hidden md:flex gap-1 font-medium opacity-90">
          <MenuBarItem
            label="File"
            items={fileMenuItems}
            className="hover:bg-black/5 dark:hover:bg-white/10 px-3 py-0.5 rounded transition-colors cursor-default"
          >
            File
          </MenuBarItem>
